...
public class StorageJSONImpl implements Storage {
	
	
	
	private String pathJSON = "C:\\Users\\nikol\\OneDrive\\Radna površina\\SK projekat\\ConverterComponent\\Data\\JSON\\data.json";
	...
	public void addEntity(Entity ent) {
		...
		int najveci = 0;
		try {
			List<Entity> Entiteti = objectMapper.readValue(new File(pathJSON), new TypeReference<List<Entity>>() {
			});
			...
			ent.setId(najveci+1);
			Entiteti.add(ent);
			exportObjectToFile(Entiteti, pathJSON);
		} catch (Exception e) {
	...
	public void removeEntities(String err, String name) {
		...
		try {
			...
			exportObjectToFile(list, pathJSON);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
